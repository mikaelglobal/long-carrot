...
const MODELS = {
  fom: { id: "deepseek-chat", name: "FOM 1.0", description: "Fast Output Model" },
  rvm: { id: "deepseek-reasoner", name: "RVM 1.0", description: "Research Verifying Model" }
};
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!response.ok) {
      ...
      return jsonResponse({ error: `HTTP ${response.status}: ${errorText}` }, response.status);
    }

    const result = await response.json();

    if (result.error) {
      return jsonResponse({ error: `API Error: ${result.error.message || JSON.stringify(result.error)}` }, 400);
    }
